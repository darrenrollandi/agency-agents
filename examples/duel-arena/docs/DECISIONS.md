...
## 07/09/2026 — Analytics: one guarded wrapper, funnel deduplicated in the profile

`AnalyticsService` is the only caller of Roblox's AnalyticsService. Every call is wrapped in pcall and gated by `Config/Analytics.Enabled`, so analytics can never break gameplay; failures are visible only as `[DEBUG]` lines. The onboarding funnel (Joined → SteppedOnPad → FirstDuel → FirstWin) is recorded once per profile via `Profile.Data.Funnel`, so a returning player never re-enters it. Step numbers are the index in `Config/Analytics.Funnel`; never reorder a shipped funnel. Economy events use the currency `XP` (Gameplay source, sku DuelWin / DuelLoss) and `BoostMinutes` (IAP source, sku XPBoost); progression path `Level`; custom events `DuelWon` / `DuelLost` with rounds won as the value. Rejected: a third-party analytics SDK (HTTP off, no dependency budget); logging from the client (spoofable).
